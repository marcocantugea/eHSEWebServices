...
Partial Class TRA_TRA_Revision_1
    ...
    Protected Sub traerTRA()
        ...
        For Each row As DataRow In dts.Tables(0).Rows
            ...
            If Not IsDBNull(row("tra_SentBack_Reason")) Then
                lblRazon.Text = "This TRA is been sent back by Safety for this reason: " & row("tra_SentBack_Reason")
    ...
    Protected Sub cargarTareaEnDiv(task_Id As Integer)
        ...
        If dts.Tables(0).Rows.Count > 0 Then
            For Each row As DataRow In dts.Tables(0).Rows
                ...
                If Not IsDBNull(row("traR_Ini_Severity")) Then
                    dbInSev.SelectedValue = row("traR_Ini_Severity")
                Else
                    dbInSev.SelectedValue = ""
                End If
                If Not IsDBNull(row("traR_Ini_Likeli")) Then
                    dbInLike.SelectedValue = row("traR_Ini_Likeli")
                Else
                    dbInLike.SelectedValue = ""
                End If
                If Not IsDBNull(row("traR_Ini_Risk")) Then
                    dbInRisk.SelectedValue = row("traR_Ini_Risk")
                Else
                    dbInRisk.SelectedValue = ""
                End If

                tMinimise.Text = row("traR_MinimizeRiskBy")

                If Not IsDBNull(row("traR_Res_Severity")) Then
                    dbResSev.SelectedValue = row("traR_Res_Severity")
                Else
                    dbResSev.SelectedValue = ""
                End If
                If Not IsDBNull(row("traR_Res_Likeli")) Then
                    dbResLike.SelectedValue = row("traR_Res_Likeli")
                Else
                    dbResLike.SelectedValue = ""
                End If
                If Not IsDBNull(row("traR_Res_Risk")) Then
                    dbResRisk.SelectedValue = row("traR_Res_Risk")
                Else
                    dbResRisk.SelectedValue = ""
                End If
    ...
    Protected Sub moverArriba(ByRef TareaId As Integer)
        ...
        'MessageBox(i)
        If i = 1 Then
            MessageBox("First Row / Primer Renglon")
        Else
            i = i - 1
    ...
    Protected Sub btnPhoneSrc0_Click(sender As Object, e As EventArgs) Handles btnPhoneSrc0.Click
        'Dar Status Safety 
        Dim strSQL As String
        Dim hoy As Date = Format(Now.Date.ToString("MM/dd/yyyy"))
        strSQL = "UPDATE traRecords SET tra_Status='Safety',tra_AuthorityApproveDate='" & hoy & "'" & _
