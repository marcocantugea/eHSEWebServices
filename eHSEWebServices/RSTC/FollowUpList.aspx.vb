...
Partial Class FollowUpList
    ...
    Protected Sub fillStopCards()
        ...
        If tID.Text.Length > 0 Then
            Sql = "SELECT * FROM tblStopCard WHERE ID=" & tID.Text & " AND Estatus like '%" & dStatus.SelectedValue & "%' AND Fecha Between #" & tDateDel.Text.Trim & "# AND #" & tDateA.Text.Trim & "# AND SOC_Relevante=Yes ORDER BY Fecha ASC"
        Else
            Sql = "SELECT * FROM tblStopCard WHERE Estatus like '%" & dStatus.SelectedValue & "%' AND Fecha Between #" & tDateDel.Text.Trim & "# AND #" & tDateA.Text.Trim & "# AND SOC_Relevante=Yes ORDER BY Fecha ASC"
        End If
