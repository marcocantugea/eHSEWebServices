...
Partial Class SOC___Summary
    ...
    Protected Sub fillStopCards()
        ...
        clsDB.Close()
        Dim Sql2 As String = "Select Empleado_ID,Nombre From ctgEmpleados WHERE Dpto_ID=" & dDpt.SelectedValue & " ORDER BY Nombre ASC"
    ...
    Protected Sub seleccionarFila(ByVal id As String)
        ...
        b = id
        Try
            Dim sql As String = "Select Nombre,Puesto,Desc,Crew,Empresa_Desc,emp_FotoRaiz,emp_FotoNombre,emp_FotoExt from qryStopEmpleados WHERE Empleado_ID=" & id & " AND Fecha Between #" & tDateDel.Text.Trim & "# AND #" & tDateA.Text.Trim & "# "
        dts = clsDB.QueryDataSet(sql)
        clsDB.Close()
        If dts.Tables(0).Rows.Count > 0 Then
            For Each row As DataRow In dts.Tables(0).Rows
                ...
                If Not IsDBNull(row("emp_FotoNombre")) Then
                    path = row("emp_FotoRaiz") & row("emp_FotoNombre") & row("emp_FotoExt")
        ...
        Dim item, codigo, revision, boton As String
        ...
        Catch ex As Exception
            Response.Write("Error in SOC - Summary Page, getting worker data; " & ex.Message)
        End Try
    End Sub
