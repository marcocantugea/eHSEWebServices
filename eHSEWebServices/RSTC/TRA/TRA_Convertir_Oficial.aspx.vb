...
Partial Class TRA_TRA_Convertir_Oficial
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        If Not Page.IsPostBack Then
           lblUser.Text = "Welcome " + Context.User.Identity.Name & "."
            ...
            traerDepartamentos()
            traerUnidades()
            txtFecha.Text = Format(Now.Date.ToString("MM/dd/yyyy"))
            ...
            If estado = "Safety" Then
                cargarTRADetalles()
                traerTRA()
            ElseIf estado = "Draft" Or estado = "Authority" Then
            ...
            ElseIf estado = "Pending_Delete" Then
                cargarTRADetalles()
                traerTRA()
                btnPhoneSrc.Visible = False
                btnRegresar.Visible = True
                btnPhoneSrc0.Enabled = False
            End If
        End If
    End Sub
    Function traerStatus(traID As Integer) As String
        Dim dts As DataSet
        Dim sql As String
        sql = "select tra_Status from traRecords where tra_ID=" & traID & ""
    ...
    Protected Sub BindRepeater() '& dbDepartamento.SelectedItem.Text &
        Dim sql As String
        'Buscar todos los documentos por Departamento
        Dim docTipo As String = dbDocTipo.SelectedItem.Value
        If docTipo = "Select One" Then
            docTipo = ""
        End If
        sql = "Select proc_Id,proc_Codigo, proc_Descripcion from traProcedimientos where proc_Departamento='" & dbProcDepartamento.SelectedItem.Value & "' and proc_TipoDoc like '%" & docTipo & "%' and proc_Descripcion like '%" & tSearchDoc.Text & "%'"
    ...
    Protected Sub GuardarTRAMain()
        ...
        txtFecha.Text = Format(Now.Date.ToString("MM/dd/yyyy")) ' ,tra_Location='" & txtLocacion.Text & "'
        strSQL = "UPDATE traRecords SET tra_Unit='" & dbUnit.SelectedItem.Text & "',tra_Dept='" & dbDepartamento.SelectedItem.Text & "'" & _
        ",tra_Activity_Job='" & txtActividad.Text.Replace("'", "''") & "',tra_Hazard_Subs_NA=" & RadioButtonList1.Items(0).Selected & "," & _
        ...
        ",tra_Status='Safety' WHERE TRA_ID = " & ltraId.Text & ""
    ...
    Protected Sub traerTRA()
        ...
        For Each row As DataRow In dts.Tables(0).Rows
            ...
            If Not IsDBNull(row("tra_Date")) Then
    ...
    Protected Sub moverAbajo(ByRef TareaId As Integer)
        ...
        If i = rptTRADetail.Items.Count Then
            MessageBox("Es el ultimo renglon. No Baja")
        Else
            i = i + 1
            Try 'Este es el renglon que deseo mover hacia abajo
                strSQL = "UPDATE traTasks SET traR_Consecutivo = " & lConsecutivoTras.Text & " WHERE traR_Consecutivo = " & i & " and tra_ID = " & ltraId.Text & ""
    ...
    Protected Sub btnPhoneSrc0_Click(sender As Object, e As EventArgs) Handles btnPhoneSrc0.Click
        Dim strSQL As String
        Dim hoy As Date = "1/1/2016"
        strSQL = "UPDATE traRecords SET tra_Status='Oficial', tra_SafetyApproveDate='" & hoy & "'" & _
        " WHERE TRA_ID = " & ltraId.Text & ""
        If clsDB.QueryExecuteNonQuery(strSQL) = True Then
            clsDB.Close()
            Response.Redirect("TRA_Enviar_Oficial.aspx?TraId=" + ltraId.Text + "&Departamento=" + dbDepartamento.SelectedItem.Text + "")
        Else
            MessageBox("Error Closing SOC. Qry=" & strSQL)
        End If
    End Sub
    ...
    Protected Sub btnRegresar_Click(sender As Object, e As EventArgs) Handles btnRegresar.Click
        ...
        tRazon.Visible = True
        'Dim strSQL As String
        'strSQL = "UPDATE traRecords SET tra_Status='Authority'" & _
        '" WHERE TRA_ID = " & ltraId.Text & ""
        'If clsDB.QueryExecuteNonQuery(strSQL) = True Then
        '    clsDB.Close()
        '    Response.Redirect("TRA_Enviar_Devolucion.aspx?TraId=" + ltraId.Text + "&Departamento=" + dbDepartamento.SelectedItem.Text + "")
        'Else
        '    MessageBox("Error Closing SOC. Qry=" & strSQL)
        'End If
    End Sub

    Protected Sub bRejectConfirm_Click(sender As Object, e As EventArgs) Handles bRejectConfirm.Click
        If Len(txtRazon.Text) > 0 Then
            Dim strSQL As String
            strSQL = "UPDATE traRecords SET tra_Status='Authority', tra_SentBack_Reason='" & txtRazon.Text & "'" & _
            " WHERE TRA_ID = " & ltraId.Text & ""
            If clsDB.QueryExecuteNonQuery(strSQL) = True Then
                clsDB.Close()
                Response.Redirect("TRA_Enviar_Devolucion.aspx?TraId=" + ltraId.Text + "&Departamento=" + dbDepartamento.SelectedItem.Text + "&Razon=" + txtRazon.Text + "")
            Else
                MessageBox("Error Closing SOC. Qry=" & strSQL)
            End If
        Else
            MessageBox("The Reject Reason cannot be empty")
        End If
        
    End Sub

    Protected Sub bRejectCancel_Click(sender As Object, e As EventArgs) Handles bRejectCancel.Click
        Me.MaintainScrollPositionOnPostBack = True
        tRazon.Visible = False
    End Sub
End Class
